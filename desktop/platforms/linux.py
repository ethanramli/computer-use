"""Linux adapter. X11 (XTEST) and Wayland (portals) are separate environments.

Not implemented on this host (no Linux/X11 or Wayland session available for
live verification). Every operation raises an explicit structured capability
error through LinuxBackend; nothing silently fakes success.
"""
...
class LinuxBackend(Backend):
    ...
    def _unsupported(self, cap):
        raise CapabilityError(
            cap,
            f"{cap} is not implemented for the Linux backend yet; "
            "requires X11 (XTEST) or a Wayland portal session",
        )
